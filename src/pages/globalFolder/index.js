...
const GlobalFolder = props => {
  ...
  const moveDirectory = (record) => {
    ...
    axios.get('/api/files/global').then(res => {
      const globalIdent = res.data._id
      axios.put('/api/files/move', { idChild: record._id, idFather: fileMovePopover || globalIdent }).then(() => {
        setNav(`/globalFolder/${fileMovePopover}`);
      }).catch((err) => {
        error(err);
        setButtonMoveFileLoading(false);
      });
  ...
  if(nav) return (<Redirect to = { nav } />);
    ...
    return (
      <MainLayout page = "globalFolder" loading = {loadingPage}>
        <Row style = {{ marginBottom: 0, display: 'flex' }}>
          <Breadcrumb>
            <Breadcrumb.Item>
              <Link to = "/globalFolder"><Icon type = "global" /></Link>
            </Breadcrumb.Item>
            { path.map((item, index) => (
              path.length === index + 1 ? (
                ...
                <Breadcrumb.Item key = { item._id }>
                  <Link to = {`/globalFolder/${item._id}`}>{item.name}</Link>
                </Breadcrumb.Item>
              )
            ))}
          </Breadcrumb>
        ...
        <Table
          ...
          onRow = { record => ({
            onDoubleClick: () => (record.type === 1 ? setNav(`/globalFolder/${record._id}`) : window.open(record.file.url, '_blank'))
          })}
        >
          <Column
            ...
            render = { (name, record) => (
              enableEditRow === record._id ? (
                ...
                <Popover
                  ...
                  content = {(
                    <div>
                      <Paragraph strong style = {{ marginBottom: 2 }}> { files[files.map(e => e._id).indexOf(fileMovePopover || globalID)].name }: </Paragraph>

                      { files && files.length ? (
                        <DirectoryTree expandedKeys = {[fileMovePopover]} onExpand = { (expandedKeys) => setFileMovePopover(expandedKeys[expandedKeys.length - 1]) }>
                          { fileMovePopover && globalID !== fileMovePopover ? (<TreeNode title = "/.." key = { files[files.map(e => e._id).indexOf(fileMovePopover)].father } />) : '' }

                          { files[files.map(e => e._id).indexOf(fileMovePopover || globalID)].children.map(child => {
                      ...
                      <Button loading = { buttonMoveFileLoading } onClick = { () => moveDirectory(record) } style = {{ width: '100%', marginTop: 10 }} disabled = { files[files.map(e => e._id).indexOf(fileMovePopover || globalID)]._id === record.father } type = "primary" size = "small"> Mover Aqui </Button>
                    </div>
                  )}
                >
